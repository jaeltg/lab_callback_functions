...
describe('Cinema', function () {
  ...
  it('should be able to filter films by year', function() {
    const actual = cinema.filterFilmsByProperty('year', 2018);
    const expected = [blackPanther];
    assert.deepStrictEqual(actual, expected);
  });
  
  it('should be able to check whether there are some films from a particular year', function() {
    const actual = cinema.checkIfFilmsFromYear(2017);
    const expected = true
    assert.strictEqual(actual, expected);
  });

  it('should be able to check whether there are no films from a particular year', function() {
    const actual = cinema.checkIfFilmsFromYear(2008);
  ...
  function() {
    const actual = cinema.checkIfMinimumLength(90);
    const expected = true;
    assert.strictEqual(actual, expected);
  });
